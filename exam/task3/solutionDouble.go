...
func main() {
	...
	printErrorAndExit(err, "Error declare a queue")
	err = ch.QueueBind(q.Name, "", "p3fanoutExchange", false, nil)
	printErrorAndExit(err, "Error to bind a queue")

	msgs, err := ch.Consume(q.Name, "Double", false, false, false, false, nil)
	...
	go func() {
		for i := 0; i < 3; i++ {
			...
			for d := range msgs {
				...
				res, err := strconv.Atoi(bodyString)
				if err != nil {
					printErrorAndExit(err, "Error to convert")
				}
				fmt.Println("Received:", bodyString, "Result:", res*2)
				d.Ack(false)
			}
			defer wg.Done()
		}
